...
/** Build a minimal EIR document with a single expression node. */
function eirDoc(expr: Record<string, unknown>) {
	return {
		version: "1.0.0",
		airDefs: [],
		nodes: [
...
// PIR expressions (forbidden in AIR, CIR, EIR, LIR)
...
describe("Layer Boundaries - Unit Tests", () => {
	...
	//==========================================================================
	// AIR rejects PIR expressions
	//==========================================================================

	describe("AIR rejects PIR expressions", () => {
	...
	//==========================================================================
	// CIR rejects PIR expressions
	//==========================================================================

	describe("CIR rejects PIR expressions", () => {
	...
	//==========================================================================
	// EIR rejects PIR expressions
	//==========================================================================

	describe("EIR rejects PIR expressions", () => {
		it("EIR should reject spawn expression", () => {
			const result = validateEIR(eirDoc(spawnExpr));
			assert.strictEqual(result.valid, false);
		});

		it("EIR should reject await expression", () => {
			const result = validateEIR(eirDoc(awaitExpr));
			assert.strictEqual(result.valid, false);
		});

		it("EIR should reject par expression", () => {
			const result = validateEIR(eirDoc(parExpr));
			assert.strictEqual(result.valid, false);
		});

		it("EIR should reject channel expression", () => {
			const result = validateEIR(eirDoc(channelExpr));
			assert.strictEqual(result.valid, false);
		});

		it("EIR should reject send expression", () => {
			const result = validateEIR(eirDoc(sendExpr));
			assert.strictEqual(result.valid, false);
		});

		it("EIR should reject recv expression", () => {
			const result = validateEIR(eirDoc(recvExpr));
			assert.strictEqual(result.valid, false);
		});

		it("EIR should reject select expression", () => {
			const result = validateEIR(eirDoc(selectExpr));
			assert.strictEqual(result.valid, false);
		});

		it("EIR should reject race expression", () => {
			const result = validateEIR(eirDoc(raceExpr));
			assert.strictEqual(result.valid, false);
		});
	});

	//==========================================================================
	// LIR expression nodes reject PIR expressions
	//
	// LIR expression nodes use CirExprSchema, so they should reject both
	// EIR-specific and PIR-specific expression kinds.
	//==========================================================================

	describe("LIR expression nodes reject PIR expressions", () => {
